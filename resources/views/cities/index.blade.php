...
<div class="discovery-wrap metro-guide">
  ...
  <section class="metro-note mb-5"><h2>東京・大阪・名古屋は、区から探せます。</h2><div class="metro-jumps">@foreach(\App\Support\WardGuide::METROS as $slug=>$info)<a href="{{ route('wards.index',$slug) }}">{{ $info['label'] }}・{{ count(\App\Support\WardGuide::wards($slug)) }}区 →</a>@endforeach</div></section>
  <h2>どの街から出かける？</h2>
