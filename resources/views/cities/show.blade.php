...
<div class="discovery-wrap metro-guide">
  ...
  <section class="mb-5" aria-labelledby="more-city-facilities"><h2 id="more-city-facilities">市内で見つかる、ほかの施設 <small>（{{ $additionalSpots->count() }}施設）</small></h2><p>所在地と紹介内容を公式サイトで照合した施設です。詳しい営業条件は各施設の公式案内をご確認ください。</p>
  <div class="metro-grid">@foreach($additionalSpots as $spot)<article class="metro-card" data-additional-city="{{ $city }}"><p class="metro-eyebrow">{{ \App\Support\Discovery::label($spot->category) }} · {{ $metadata['label'] }}</p><h3><a href="{{ route('spots.show',$spot) }}">{{ $spot->name }}</a></h3><p>{{ $spot->description }}</p><p><strong>所在地：</strong>{{ $spot->address }}</p>@if($spot->source_checked_at)<p class="small">公式確認：{{ $spot->source_checked_at->format('Y-m-d') }}</p>@endif<a class="discovery-button" href="{{ route('spots.show',$spot) }}">施設情報・体験記を見る →</a></article>@endforeach</div></section>
  @endif
