...
@section('title', $area . 'のアウトドア・癒やし施設' . $spots->count() . '件 | ' . config('app.name'))
@section('description', $area . 'のアウトドア・癒やし施設施設を' . $spots->count() . '件掲載。混雑の参考情報の報告と口コミを確認できます。新しい施設は誰でも投稿できます。')
...
{!! json_encode([
  ...
  '@type' => 'CollectionPage',
  'name' => $area . 'のアウトドア・癒やし施設',
  'url' => url('/areas/' . rawurlencode($area)),
  'description' => $area . 'のアウトドア・癒やし施設施設の一覧。',
...
<div class="container my-4">
  <div class="card shadow-sm mb-3">
    <div class="card-body p-4">
      <h1 class="h3 fw-bold mb-3">{{ $area }}のアウトドア・癒やし施設</h1>
      <p class="text-muted mb-2">
        {{ $area }}の施設を{{ $spots->count() }}件掲載しています。
        @if($glampingCount > 0)
          このうち{{ $glampingCount }}件はグランピング施設です。
        @endif
        混雑の参考情報は利用者からの報告で更新されます。
      </p>
      <p class="text-muted small mb-3">
        混雑の参考情報と口コミは利用者の投稿によるもので、施設の公式発表ではありません。
      ...
      <a href="{{ route('spots.index') }}" class="btn btn-outline-secondary btn-sm">施設を探す</a>
    </div>
  </div>
      ...
      <div class="col-12 col-md-6">
        <div class="card h-100 shadow-sm">
          <div class="card-body">
            ...
            @endif
            <small class="text-muted d-block mb-1">混雑の参考情報：{{ \App\Helpers\CongestionHelper::getText($spot->average_congestion) }}</small>
            @if($spot->booking_url)
